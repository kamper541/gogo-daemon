            ...
            if (document.location.search.length) {
                ...
                $.each(document.location.search.replace(/^\?/,'').split('&'),
                    function(index,value){
                        ...
                        //Add import method
                        if (name=='importjson'){
                            $.rib.pmUtils.importProjectCustom(args,null,function (){alert("Invalid file.");});
                            console.log(args);
                        }
                    }
                );
...
$(function() {
    ...
    function fsInitSuccess(fs) {
        ...
        $('body').append('<div id="dialog-form" title="Export to Gogo" class="vbox ui-dialog-content ui-widget-content" style="display:none;">\
            <div class="vbox ui-dialog-content ui-widget-content" style="padding: 0.8em 1.2em;">\
              <form id="export_form">\
    <p>\
      <label for="name">File name</label>\
      <input type="text" name="filename" id="filename" value="" class="text ui-widget-content ui-corner-all" required> .html\
      <!-- Allow form submission with keyboard without duplicating the dialog button -->\
      <input type="submit" tabindex="-1" style="position:absolute; top:-1000px" class="buttonStyle">\
    </p>\
  </form>\
  </div>\
</div>');
        ...
        $(document).delegate('#exportProjCust', "click",
            $.rib.pmUtils.exportProjectCustom);
        ...
        $(document).delegate('#export_form', "submit", function (e) {
            e.preventDefault();
            $.rib.uploadToGoGo();
        });

...
$(function () {
    ...
    function exportPackageCustom(ribFile) {
        var exportDialog, pid;
        pid = pid || $.rib.pmUtils.getActive();
     ...
     * @return {None} as same as addCustomFile.
     */
    function uploadToGoGo(json_content){
        /*
        var $selectedTab = $('li.ui-tabs-selected a');
        $('.codeViewButton').click();
        var html_content = $('.CodeMirror-lines div').text();
        html_content = html_content.substring(html_content.lastIndexOf("<!DOCTYPE html>"));
        $selectedTab.click();
        */

        var html_content = $.rib.generateHTML(null, false, function (admNode, domNode) {
                var matched, p;
                matched = admNode.getMatchingProperties($.rib.pmUtils.relativeFilter);
                // Add uploaded images to the needed list
                for (p in matched) {
        ...
        if ($('#filename').val()==''){
            ...
            postToGoGo($('#filename').val(), html_content, false, json_content);
        }
    }
     ...
     * @return {None} as same as addCustomFile.
     */
    function postToGoGo(filename, html_content, confirm_status, json_content){

        if (!json_content){
            json_content = $.rib.jsonContent;
        }
        $.post( "rib_upload.html",
            { filename: filename, html_content: html_content, confirm: confirm_status, json: json_content }, 
                function(data) {
                    //var data = $.parseJSON(data);
                    if (data.result == false && data.message == "file_exists"){
                        //confirm
                        if(confirm("Already has a file named \"" + filename + ".html\" \n Do you want to overwrite?")){
                            postToGoGo(filename, html_content, true, json_content);
                        }
                    } else if (data.result != false || data.result == 0){
                        $( "#dialog-form" ).dialog( "close" );
                        alert("Export to a file named '" + filename + ".html' Complete!!");
                    }
                  //console.log( data );
                })
              .fail(function() {
    ...
    //Custom functions
    $.rib.exportPackageCustom = exportPackageCustom;
    $.rib.uploadToGoGo = uploadToGoGo;
    $.rib.postToGoGo = postToGoGo;
    $.rib.jsonContent = jsonContent;
